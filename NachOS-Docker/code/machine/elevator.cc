...
public:
    ...
    bool MoveTo(int goingToFloor) {
        ...
        if ( !inMotion ) {
            willArrive = kernel->stats->totalTicks + DelayPerFloor;
            inMotion = TRUE;
            return TRUE;
        }
        return FALSE;
    ...
    bool Enter(int onFloor) {  // rider enters the elevator
        if ( !doorsOpen || lastFloor != onFloor ) {
            ...
            ASSERT(!inMotion);
            ASSERT((signed)riders->NumInList() < MaxRiders);
            riders->Append(kernel->currentThread);
            return TRUE;
        }
    }
    bool Exit(int onFloor) {  // rider leaves the elevator
        if ( !doorsOpen || lastFloor != onFloor ) {
            ...
            ASSERT(!inMotion);
            riders->Remove(kernel->currentThread);
            return TRUE;
        }
    }

    bool ReachedNextFloor() {  // has the elevator reached the next floor?
        if ( inMotion && (kernel->stats->totalTicks >= willArrive) ) {
    ...
    List<Thread *> *riders;  // who is on board?
};
...
ElevatorBank::ElevatorBank(int numElvtr, int numFlr, CallBackObj *riders,
                           CallBackObj *controllers) {
    numElevators = numElvtr;
    numFloors = numFlr;
    callRiders = riders;
    callControllers = controllers;
...
void ElevatorBank::OpenDoors(int elevator) {
    ...
    PostEvent(riderEvents, DoorsOpened, elevators[elevator]->OpenDoors(), elevator, FALSE);
}
...
void ElevatorBank::MoveTo(int goingToFloor, int elevator) {
    ...
    if ( elevators[elevator]->MoveTo(goingToFloor) ) {
        kernel->interrupt->Schedule(this, DelayPerFloor, ElevatorInt);
    }
}
...
void ElevatorBank::PressButton(int onFloor, Direction goingTo) {
    ...
    PostEvent(controllerEvents, goingTo == Up ? UpButtonPressed : DownButtonPressed, onFloor,
...
void ElevatorBank::PressFloor(int goingToFloor, int elevator) {
    ASSERT(goingToFloor >= 0 && goingToFloor < numFloors);
    ASSERT(elevator >= 0 && elevator < numElevators);
    ASSERT(elevators[elevator]->riders->IsInList(kernel->currentThread));
    PostEvent(controllerEvents, FloorButtonPressed, goingToFloor, elevator, FALSE);
}
    ...
    for ( int i = 0; i < numElevators; i++ ) {
        if ( elevators[i]->ReachedNextFloor() ) {
            if ( elevators[i]->CheckArrived() ) {
                PostEvent(controllerEvents, ElevatorArrived, elevators[i]->goingTo, i, TRUE);
            } else if ( !intSched ) {
...
ElevatorEvent ElevatorBank::getNextEvent(ListOfEvents *list, int *floor, int *elevator) {
...
void ElevatorBank::PostEvent(ListOfEvents *list, ElevatorEvent ev, int floor, int elevator,
                             bool inHandler) {
    ...
    if ( !inHandler ) {
        kernel->interrupt->Schedule(this, 1, ElevatorInt);
    }
}
