...
static char *intTypeNames[] = {"timer",    "disk",         "console write", "console read",
                               "elevator", "network send", "network recv"};
...
void Interrupt::Schedule(CallBackObj *toCall, int fromNow, IntType type) {
    ...
    DEBUG(dbgInt,
          "Scheduling interrupt handler the " << intTypeNames[type] << " at time = " << when);
    ASSERT(fromNow > 0);
