...
private:
    ...
    ElevatorEvent WaitForNextRiderEvent(int *floor, int *elevator) {
        ...
        while ( (event = elevators->getNextRiderEvent(floor, elevator)) == NoEvent ) {
            riderWakeup->P();
        }
        return event;
    }
    ElevatorEvent WaitForNextControllerEvent(int *floor, int *elevator) {
        ...
        while ( (event = elevators->getNextControllerEvent(floor, elevator)) == NoEvent ) {
            controllerWakeup->P();
        }
        return event;
    }
...
static void ControllerTest(ElevatorInspector *inspector) { inspector->ControllerTest(); }
