...
char *symbol_type[] = {"Nil",      "Global",  "Static",     "Param",   "Local", "Label",
                       "Proc",     "Block",   "End",        "Member",  "Type",  "File",
                       "Register", "Forward", "StaticProc", "Constant"};

char *storage_class[] = {"Nil",        "Text",        "Data",    "Bss",        "Register", "Abs",
...
char *argv[];
{
    ...
    if ( !read_struct(f, filehdr) || !read_struct(f, aouthdr) || filehdr.f_magic != MIPSELMAGIC ) {
...
print_section(i) int i;
{
    ...
    printf("Section: %s\t%d/%d\n", scnhdr[i].s_name, scnhdr[i].s_size, section[i].relocs);
...
char *reloc_type[] = {"abs", "16", "32", "26", "hi16", "lo16", "gpdata", "gplit"};
